...
// ✅ Fetch coffins only where transfer_location is filled (non-null and non-empty)
$stmt = $conn->prepare("SELECT * FROM coffins WHERE transfer_location IS NOT NULL AND TRIM(transfer_location) != ''");
$stmt->execute();
...
<html>
<head>
    <title>Transferred Coffins</title>
...
<table id="coffinTable" class="display nowrap">
    ...
    <tbody>
        ...
        <tr data-id="<?= $row['id'] ?>">
            ...
            <td><?= htmlspecialchars($row['status']) ?></td> <!-- ✅ Keep this -->
